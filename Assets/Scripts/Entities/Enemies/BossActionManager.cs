...
public class BossActionManager : EntityActionManager
{
    ...
    private float _timeBeforeChangingDestination = 5f;
    private float _timeBeforeTeleport = 35f;
    ...
    private void Update()
    {
        if (_timeBeforeTeleport > 0f) _timeBeforeTeleport -= Time.deltaTime;
    ...
    private void TryToAttack(float distanceToTarget)
    {
        ...
        {
            if (Self.spellBook.SpaceSpell[0].IsReady())
            {
                Stop();
                Self.spellManager.CastSpell(Self.spellBook.SpaceSpell[0], CurrentTarget.transform.position);
                _timeBeforeTeleport += 3f;
            }
            else if (Self.spellBook.ASpell[0].IsReady() && _timeBeforeTeleport <= 0f)
            {
                Stop();
                Self.spellManager.CastSpell(Self.spellBook.ASpell[0], CurrentTarget.transform.position);
            }
            else
            {
    ...
    {
        if (_timeBeforeChangingDestination <= 0f) 
        {
            ...
            _timeBeforeChangingDestination = 5f;
            StartCoroutine(UpdateDestination());
        }
